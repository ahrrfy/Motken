...
-   **Student Management**: Stats cards, profile summary, batch actions, archive/restore, notes, enhanced export. **Study Mode (studyMode)**: Each student has `in-person` (حضوري, default) or `online` (إلكتروني) study mode. Cyan badge "إلكتروني" with colored name appears in table/profile. Filter by study mode. Batch conversion (select students → convert to online/in-person). Supervisors and admin can manage.
