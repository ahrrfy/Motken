...
-   **Security**: Helmet HTTP security headers (HSTS, Referrer-Policy, formAction, upgradeInsecureRequests), Scrypt password hashing with timing-safe comparison, session security (custom cookie name `mutqin.sid`, session regeneration on login, full session destruction on logout, 7-day cookie TTL), login/API rate limiting (5 attempts/15min per IP+user, auth rate limiter 20/15min), IP banning, IDOR prevention, request body size limits (500kb), random default passwords, input validation (password max 128 chars), bulk operation limits (200 items). Deep security audit: mosque isolation on ALL endpoints (including attendance, communication-log, bulk-attendance with per-student verification), PII masking (teachers see only their students' phones; students see only own data), mass assignment protection with field whitelists on all PATCH endpoints, role gating on all mutation endpoints, XSS sanitization in print utilities, content text filtering. Auto session invalidation on user deactivation/suspension. Security audit logging for role/status changes. Login attempt memory cleanup (5-min interval). Seed endpoint protected (requires admin if users exist). Database indexes on all foreign key columns (27+ tables) for DoS prevention. Points validation (max ±10000, reason max 500 chars). x-powered-by disabled. Error message sanitization (zero err.message leaks to client, all 41+ endpoints sanitized). XSS escapeHtml applied to all print templates (ID cards, certificates, semester reports, annual summaries). Donations page redesigned (removed fake payment form).
