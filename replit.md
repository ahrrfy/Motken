...
-   **Assignments & Exams**: Stats cards, 1-100 grading, deadline tracking, visual calendar, completion rates, bulk assignment. **NEW: Quick-grade preset buttons (ممتاز/جيد جداً/جيد/مقبول/ضعيف), Smart assignment suggestion (auto-suggests next surah/verses), Auto-review assignment creation on low grades (<60).**
...
-   **Security**: Deep penetration test completed. Helmet HTTP security headers (HSTS with preload, Referrer-Policy, formAction, upgradeInsecureRequests, X-Frame-Options DENY, X-Download-Options, DNS Prefetch Control), Scrypt password hashing with timing-safe comparison, session security (custom cookie name `mutqin.sid`, session regeneration on login, full session destruction on logout, 7-day cookie TTL), login/API rate limiting (5 attempts/15min per IP+user, auth rate limiter 20/15min, sensitive action limiter 10/15min for password changes), IP banning, IDOR prevention, request body size limits (500kb), random default passwords, input validation (password max 128 chars), bulk operation limits (200 items). Deep security audit: mosque isolation on ALL endpoints (including attendance, communication-log, bulk-attendance with per-student verification), PII masking (teachers see only their students' phones; students see only own data), mass assignment protection with field whitelists on all PATCH endpoints, role gating on all mutation endpoints, XSS sanitization in print utilities, content text filtering. Auto session invalidation on user deactivation/suspension. Security audit logging for role/status changes, password changes, user deletions, failed login warnings. Login attempt memory cleanup (5-min interval). Seed endpoint protected (requires admin if users exist). Database indexes on all foreign key columns (52+ indexes across 27+ tables) for DoS prevention. Points validation (max ±10000, reason max 500 chars). x-powered-by disabled. Error message sanitization (zero err.message leaks to client, all 41+ endpoints sanitized). XSS escapeHtml applied to all print templates (ID cards, certificates, semester reports, annual summaries). Donations page redesigned (removed fake payment form). CSRF protection via Origin/Referer validation on all mutation endpoints. Content-Security-Policy (strict in production, dev-relaxed). Permissions-Policy (camera/microphone/geolocation/payment disabled). Sensitive path blocking (.env, .git, wp-admin, phpinfo). API response body logging removed (data leak prevention). Statement/query timeouts (30s). Resilient per-statement index creation.
...
-   **Teacher Intelligence**: Performance metrics (grading speed, avg grades, assignment frequency), teacher comparison dashboard for supervisors, auto-generated teaching recommendations based on student weak spots.
...
-   **Message Templates**: Predefined message templates (congratulation, warning, absence notice), communication log tracking per student.
